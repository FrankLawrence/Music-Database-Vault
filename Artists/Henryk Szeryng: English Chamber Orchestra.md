...
WHERE banner != null AND Artist = [[Henryk Szeryng: English Chamber Orchestra]]
SORT year asc
```
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Artist && p.Artist.path && p.Artist.path.includes("Henryk Szeryng: English Chamber Orchestra"))
  .map(p => {
