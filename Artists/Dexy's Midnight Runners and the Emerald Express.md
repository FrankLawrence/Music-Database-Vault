...
WHERE banner != null AND Artist = [[Dexy's Midnight Runners and the Emerald Express]]
SORT year asc
```
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Artist && p.Artist.path && p.Artist.path.includes("Dexy's Midnight Runners and the Emerald Express"))
  .map(p => {
