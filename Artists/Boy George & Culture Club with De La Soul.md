...
WHERE banner != null AND Artist = [[Boy George & Culture Club with De La Soul]]
SORT year asc
```
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Artist && p.Artist.path && p.Artist.path.includes("Boy George & Culture Club with De La Soul"))
  .map(p => {
