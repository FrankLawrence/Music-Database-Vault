...
```dataview
TABLE FROM [[Chou Ikimonobakari Tennen Kinen Members (2014)]]
```
```dataviewjs
const calendarData = {
  colors: {
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Album && p.Album.path && p.Album.path.includes("Chou Ikimonobakari Tennen Kinen Members (2014)"))
  .map(p => {
