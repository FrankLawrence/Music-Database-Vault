...
```dataview
TABLE FROM [[National Lampoon's Vacation Original Motion Picture Sound Track (1989)]]
```
```dataviewjs
const calendarData = {
  colors: {
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Album && p.Album.path && p.Album.path.includes("National Lampoon's Vacation Original Motion Picture Sound Track (1989)"))
  .map(p => {
