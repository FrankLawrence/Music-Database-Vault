...
[Artist:: [[Eric Clapton]]]
```dataview
TABLE FROM [[Rush: Music from the Motion Picture Soundtrack (1992)]]
```
```dataviewjs
const calendarData = {
  colors: {
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Album && p.Album.path && p.Album.path.includes("Rush: Music from the Motion Picture Soundtrack (1992)"))
  .map(p => {
