...
[Artist:: [[Don Felder]]]
```dataview
TABLE FROM [[Heavy Metal: Music from the Motion Picture (1981)]]
```
```dataviewjs
const calendarData = {
  colors: {
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Album && p.Album.path && p.Album.path.includes("Heavy Metal: Music from the Motion Picture (1981)"))
  .map(p => {
