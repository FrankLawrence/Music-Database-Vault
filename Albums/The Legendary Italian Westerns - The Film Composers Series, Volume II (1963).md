---
tags: Album
banner: "![[The Legendary Italian Westerns - The Film Composers Series, Volume II (1963).jpg]]"
---
[Year:: 1963]
[Artist:: [[ENNIO MORRICONE]]]
```dataview
TABLE FROM [[The Legendary Italian Westerns - The Film Composers Series, Volume II (1963)]]
```
```dataviewjs
const calendarData = {
  colors: {
    blue: ["#9ccfd8", "#5BAAB8", "#57A1BB", "#5da8c7", "#3e8fb0"]
  },
  entries: []
};

const albumSongs = dv.pages('"Songs"')
  .where(p => p.Album && p.Album.path && p.Album.path.includes("The Legendary Italian Westerns - The Film Composers Series, Volume II (1963)"))
  .map(p => {
  let songTitle = p.file.name.split(".")[0];
  return songTitle.replace(/ /g, "_");
});
for (let page of dv.pages('"Daily Notes"')) {
  let totalPlays = 0;
  for (let song of albumSongs) {
    totalPlays += page[song] || 0;
  }
  if (totalPlays > 0) {
    calendarData.entries.push({
      date: page.file.name,
      intensity: totalPlays
    });
  }
}

renderHeatmapCalendar(this.container, calendarData);
```
