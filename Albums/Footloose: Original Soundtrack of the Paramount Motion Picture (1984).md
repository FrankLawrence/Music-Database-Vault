...
```dataview
TABLE FROM [[Footloose: Original Soundtrack of the Paramount Motion Picture (1984)]]
```
```dataviewjs
const calendarData = {
  colors: {
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Album && p.Album.path && p.Album.path.includes("Footloose: Original Soundtrack of the Paramount Motion Picture (1984)"))
  .map(p => {
