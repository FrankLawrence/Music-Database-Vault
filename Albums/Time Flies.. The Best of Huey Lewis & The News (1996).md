...
```dataview
TABLE FROM [[Time Flies.. The Best of Huey Lewis & The News (1996)]]
```
```dataviewjs
const calendarData = {
  colors: {
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Album && p.Album.path && p.Album.path.includes("Time Flies.. The Best of Huey Lewis & The News (1996)"))
  .map(p => {
