...
[Artist:: [[Coldplay]]]
```dataview
TABLE FROM [[Viva La Vida Or Death And All His Friends (2008)]]
```
```dataviewjs
const calendarData = {
  colors: {
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Album && p.Album.path && p.Album.path.includes("Viva La Vida Or Death And All His Friends (2008)"))
  .map(p => {
