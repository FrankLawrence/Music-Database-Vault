...
```dataview
TABLE FROM [[Chou Ikimonobakari Tennen Kinen Members (2008)]]
```
```dataviewjs
const calendarData = {
  colors: {
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Album && p.Album.path && p.Album.path.includes("Chou Ikimonobakari Tennen Kinen Members (2008)"))
  .map(p => {
