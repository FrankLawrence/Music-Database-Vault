...
```dataview
TABLE FROM [[Rolled Gold Plus: Very Best Of The Rolling Stones [Disc 2] (1971)]]
```
```dataviewjs
const calendarData = {
  colors: {
...
const albumSongs = dv.pages('"Songs"')
  .where(p => p.Album && p.Album.path && p.Album.path.includes("Rolled Gold Plus: Very Best Of The Rolling Stones [Disc 2] (1971)"))
  .map(p => {
